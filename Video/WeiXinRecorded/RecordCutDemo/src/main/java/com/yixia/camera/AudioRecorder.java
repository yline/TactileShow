...
public class AudioRecorder extends Thread {
	...
	public AudioRecorder(MediaRecordCallback mediaRecorder) {
	...
	public void run() {
		if (mSampleRate != 8000 && mSampleRate != 16000 && mSampleRate != 22050 && mSampleRate != 44100) {
			mMediaRecorder.onRecordAudioError(MediaRecorderBase.AUDIO_RECORD_ERROR_SAMPLERATE_NOT_SUPPORT, "sampleRate not support.");
			return;
		}
		...
		if (AudioRecord.ERROR_BAD_VALUE == mMinBufferSize) {
			mMediaRecorder.onRecordAudioError(MediaRecorderBase.AUDIO_RECORD_ERROR_GET_MIN_BUFFER_SIZE_NOT_SUPPORT, "parameters are not supported by the hardware.");
			return;
		}
		...
		if (null == mAudioRecord) {
			mMediaRecorder.onRecordAudioError(MediaRecorderBase.AUDIO_RECORD_ERROR_CREATE_FAILED, "new AudioRecord failed.");
			return;
		}
		...
		try {
			...
			mMediaRecorder.onRecordAudioError(MediaRecorderBase.AUDIO_RECORD_ERROR_UNKNOWN, "startRecording failed.");
			return;
		}
		...
		try {
			...
			mMediaRecorder.onRecordAudioError(MediaRecorderBase.AUDIO_RECORD_ERROR_UNKNOWN, message);
		}
